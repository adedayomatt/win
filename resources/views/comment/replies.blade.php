    ...
    <div class="collapse comment-replies px-2" id="comment-{{$comment->id}}-replies">
        <div class="list-group image-bullet" >
            ...
            @foreach($replies as $reply)
                <div class="list-group-item" id="{{$reply->user->username}}-comment-{{$reply->id}}" style="background-color: inherit">
                    <div style="background-color: #fff;padding: 5px; border-radius: 5px; margin-bottom: 5px">
                        <div class="d-flex">
                            <img src="{{$reply->user->avatar()['src']}}" alt="{{$reply->user->avatar()['alt']}}" class="avatar avatar-sm">
                            <div class="ml-2 pt-1" >
